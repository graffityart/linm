...
<section className="subhero container"><div><span className="eyebrow">린M 아이템</span><h1>실제 상점 화면에서 확인한 린M 아이템</h1><p>검색어에 맞춰 임의의 아이템을 추가하지 않고, 제공받은 게임 스크린샷에서 명칭을 확인한 항목부터 분류합니다.</p></div><SiteImage src="/images/items/items-hero.webp" alt="린M 주요 아이템 대표 이미지" priority/></section>
...
<section className="section container split-section"><SiteImage className="content-image-slot" src="/images/items/items-content-01.webp" alt="린M 상점 아이템 안내 이미지"/><div><span className="eyebrow">아이템을 볼 때</span><h2>이름만 같아도 판매처와 조건은 다를 수 있습니다</h2><p>상점 카테고리, 요구 재화, 구매 제한, 이벤트 기간을 함께 확인하는 구조로 상세 정보를 확장할 예정입니다.</p><Link className="text-link" href="/rinm/guide/growth">린M 육성 가이드 →</Link><br/><Link className="text-link" href="/rinm/guide/equipment">린M 장비 가이드 →</Link></div></section>
</>}
