...
<section className="subhero container"><div><span className="eyebrow">린M 장비</span><h1>장비 공략은 강화·제련 화면을 기준으로</h1><p>현재는 상점에서 확인된 장비 관련 아이템 명칭을 정확히 정리하고, 강화 확률이나 최적 장비처럼 근거가 필요한 정보는 추가 자료 확보 후 작성합니다.</p></div><div className="image-slot"><span>/public/images/guide/equipment-hero.webp</span></div></section>
...
<section className="section container split-section"><div className="image-slot content-image-slot"><span>/public/images/guide/equipment-content.webp</span></div><div><span className="eyebrow">관련 페이지</span><h2>재료 목록은 아이템 페이지와 연결</h2><p>장비 페이지는 사용 방식과 시스템을 설명하고, 전체 아이템 목록은 별도 허브에서 관리해 중복을 줄입니다.</p><Link className="text-link" href="/rinm/items">린M 아이템 정보 →</Link></div></section>
</>}
