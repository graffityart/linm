...
<section className="subhero container"><div><span className="eyebrow">린M 신격</span><h1>신격 추천·조합·티어는 실제 데이터부터</h1><p>신격은 핵심 검색 주제로 유지하되 현재 대화에서 확보된 자료만으로 효과·순위·계승 조건을 확정할 수 없어 근거 없는 공략은 작성하지 않습니다.</p></div><SiteImage src="/images/shingyeok/shingyeok-hero.webp" alt="린M 신격 대표 이미지" priority/></section>
...
<section className="section container split-section"><SiteImage className="content-image-slot" src="/images/shingyeok/shingyeok-content-01.webp" alt="린M 신격 편성 분위기 이미지"/><div><span className="eyebrow">업데이트 관리</span><h2>티어와 추천에는 기준 날짜를 함께 표시</h2><p>향후 신격 공략을 작성할 때는 패치로 정보가 달라지는 문제를 줄이기 위해 확인 날짜와 적용 버전을 함께 관리하는 구조로 만들겠습니다.</p></div></section>
</>}
